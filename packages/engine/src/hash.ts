/** SHA-256 in plain TypeScript, so the record can be sealed anywhere the engine runs, with no runtime dependency. */
...
export function sha256(text: string): string {
  const bytes = new TextEncoder().encode(text); const l = bytes.length; const bitLen = l * 8;
