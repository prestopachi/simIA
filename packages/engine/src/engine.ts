...
export class Town {
  ...
  private async judgeDeeds(): Promise<void> {
    ...
    await Promise.all(batch.map(async ({ a, what, with: b, place }) => {
      ...
      const name = a.persona.name;
      if (!j.plausible) { this.remember(a, `I tried to ${what}. ${j.happened}`, 0.4); this.emitFromBrain(j, "agent.do", [a.id], place.id, `${name} tried to ${what}: ${j.happened}`, 0.3, { what, happened: j.happened, plausible: false }); return; }
      ...
      this.emitFromBrain(j, "agent.do", [a.id, ...(b ? [b.id] : [])], place.id, `${name}: ${what}. ${j.happened}${spent ? ` (${spent} coins)` : ""}${gained ? ` (now has ${gained})` : ""}`, 0.45, { what, happened: j.happened, spent, gained });
    }));
  }
